...
// TestFiberServerCollapseComponentResponses verifies that the
// collapse-component-responses option produces code that compiles and works
// correctly at runtime. The collapsed server returns shared component types
// (e.g. BadrequestResponse, ReusableresponseJSONResponse) directly instead of
// per-operation wrapper types.
func TestFiberServerCollapseComponentResponses(t *testing.T) {
	server := StrictServer{}
